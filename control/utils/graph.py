...
try:
    ...
    if chosen_font:
        ...
        print("⚠️ Warning: Could not find 'Noto Color Emoji', 'Segoe UI Emoji', or 'Apple Color Emoji'. Emojis in plots might not render correctly.")
    ...
    try:
        with open(filepath, 'r') as f:
            for line_number, line in enumerate(f, 1):
                ...
                if logging_active:
                    if ":" in line and (line.startswith("ON_ms:") or line.startswith("OFF_ms:")):
                        try:
                            ...
                            rpm = 60000.0 / (angular_resolution_segments * event_duration_ms)
...
def plot_and_save_rpm(timestamps_s, rpms, angular_resolution_segments, input_filepath, output_filepath, start_time=None, end_time=None):
    """
...
def main():
    ...
    for input_file in files_to_process:
        ...
        timestamps, rpms_data, source_lines = process_log_file(input_file, args.angular_resolution_segments)

        if timestamps and rpms_data and source_lines:
            print(f"✔️ Successfully processed {len(timestamps)} data points from '{base_name}'.")

            plot_and_save_rpm(
                timestamps, rpms_data, args.angular_resolution_segments,
                input_file, output_graph_filepath,
