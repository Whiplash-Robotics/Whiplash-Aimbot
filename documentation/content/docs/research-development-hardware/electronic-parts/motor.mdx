...
## Specifications
<Callout type="warn"> WARNING! Math zone ahead. Please click [HERE](./motor#selection) to skip!</Callout>
...
\Delta d_1 = \frac{1}{2} \cdot at^2

\newline

\Delta d_2 = v_{f1} \cdot t - \frac{1}{2} at^2
...
\Delta d = \frac{1}{2} \cdot at^2 + at \cdot t - \frac{1}{2} \cdot at^2

\newline

a = \frac{0.35}{t^2} = \frac{0.35}{0.05^2} = 140m/s^2
```
#### Force
...
*credits: https://blog.orientalmotor.com/motor-sizing-basics-part-1-load-torque*
...
\omega = \frac{7}{0.03} = 233.33 \text{ rad}/s = 2228 rpm
...
Well, more like *option* (singular). After searching far and wide for many hours, almost every motor either has a crazy shipping fee, way too underpowered, too expensive, or too overpowered. Except one.
...
After doing this research, we realized that our goal of 350mm in 100ms might be unfeasible. We found a weaker motor that is less power demanding but should stil be able to deliver on our goal of *inhuman aim*.
#### 895 DC Motor
